...
export function tick(
  ...
  const hunger = Math.max(0, state.hunger - minutes * DECAY_RATES.hunger);
  const happiness = Math.max(
    0,
    state.happiness - minutes * DECAY_RATES.happiness
  );
  const energy = Math.max(0, state.energy - minutes * DECAY_RATES.energy);
  let health = state.health;
