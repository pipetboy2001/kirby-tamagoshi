...
export function saveIsPlaying(isPlaying: boolean) {
  try {
    localStorage.setItem(PLAYING_KEY, JSON.stringify(isPlaying));
  } catch (e) {
    console.warn("❌ Error guardando isPlaying:", e);
  }
}

export function loadIsPlaying(): boolean {
  try {
    return JSON.parse(localStorage.getItem(PLAYING_KEY) || "false");
  } catch {
    return false;
  }
}
