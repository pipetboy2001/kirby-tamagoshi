...
export function getKirbyEmotion(
...
export function getKirbyGif(
  ...
  if (currentAction === "eating")
    return KIRBY_GIFS.eating[currentGifIndex % KIRBY_GIFS.eating.length];
  if (currentAction === "playing")
    return KIRBY_GIFS.playing[currentGifIndex % KIRBY_GIFS.playing.length];
  if (currentAction === "sleeping")
    return KIRBY_GIFS.sleeping[currentGifIndex % KIRBY_GIFS.sleeping.length];
  if (emotion.gifType && Object.hasOwn(KIRBY_GIFS, emotion.gifType)) {
